...
module axi_spi (
    ...
    // ============================================================
    reg [7:0] tx_data_reg;
    reg       tx_start_reg; // Agora usado apenas como um pulso de controle
    reg       busy;
    reg       bvalid_reg;
    reg [7:0] rx_data_reg;
    ...
    // ============================================================
    // LÓGICA UNIFICADA (Interface AXI + SPI Master)
    // ============================================================
    always @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            // Reseta AXI
            ...
            if (!busy && s_axi_awvalid && s_axi_wvalid && !bvalid_reg) begin
                if (s_axi_awaddr[3:0] == 4'h0) begin
                    tx_data_reg  <= s_axi_wdata[7:0];
                    tx_start_reg <= 1'b1; // Pulso para iniciar SPI
                end
                bvalid_reg <= 1'b1;
            end
            ...
            if (!busy) begin
                ...
                if (tx_start_reg) begin
                    busy         <= 1'b1;
                    tx_start_reg <= 1'b0; // Consome o pulso
                    spi_cs       <= 1'b0;
                    spi_sck      <= 1'b0;
                    shift_tx     <= tx_data_reg;
                    shift_rx     <= 8'b0;
                ...
                if (tick) begin
                    ...
                    if (spi_sck) begin // Falling Edge (SCK estava em 1 e vai para 0)
                        shift_rx <= {shift_rx[6:0], spi_miso};
                    ...
                    end else begin // Rising Edge (SCK estava em 0 e vai para 1)
                        // Prepara o próximo bit para o mosi
                        spi_mosi <= shift_tx[6];
                        shift_tx <= {shift_tx[6:0], 1'b0};
                    end
                end
