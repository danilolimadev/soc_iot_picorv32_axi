...
module axi_timer (
    ...
    // Wire auxiliar para sinalizar que o processador quer limpar a interrupção
    wire clear_irq = (s_axi_awvalid && s_axi_wvalid && s_axi_awready && s_axi_wready && 
                     (s_axi_awaddr[3:0] == 4'h0) && s_axi_wdata[1]);

    // =========================================================
    // AXI write & Lógica de Controle
    // =========================================================
    always @(posedge clk) begin
        if (!resetn) begin
            s_axi_awready <= 0;
    ...
    // =========================================================
    // Lógica Unificada do Contador e IRQ (Resolve o conflito)
    // =========================================================
    always @(posedge clk) begin
        if (!resetn) begin
            counter     <= 0;
            ...
            // Lógica da Interrupção (Set e Clear no mesmo bloco)
            if (clear_irq) begin
                irq_pending <= 0;
            end else if (enable && (counter >= reload)) begin
                irq_pending <= 1;
            end
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            ...
            if (s_axi_arvalid && s_axi_arready) begin
                ...
                s_axi_rresp  <= 2'b00;
                case (s_axi_araddr[3:0])
                    4'h0: s_axi_rdata <= {30'd0, irq_pending, enable}; // bit1=irq, bit0=en
                    4'h4: s_axi_rdata <= reload;
                    4'h8: s_axi_rdata <= {31'd0, irq_pending};
                    default: s_axi_rdata <= 32'hDEADBEEF;
                endcase
