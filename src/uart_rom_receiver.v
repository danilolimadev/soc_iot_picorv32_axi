...
*/
// ============================================================
// UART ROM Receiver - REPARADO PARA SÍNTESE
// ============================================================
...
module uart_rom_receiver #(
    ...
    // NOVAS PORTAS PARA SÍNTESE (Acesso pelo Boot Manager)
            ...
            if (baud_tick) begin
                case (rx_state)
                    ...
                    RX_DATA: begin
                        rx_shift <= {uart_rx, rx_shift[7:1]};
                        rx_bitcount <= rx_bitcount + 1;
                        if (rx_bitcount == 7) rx_state <= RX_STOP;
                    end
                    RX_STOP: begin
                        rx_state <= RX_IDLE;
                        rx_ready <= 1;
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            ...
            if (rx_ready && !done) begin
                // Lógica corrigida para montar a palavra de 32 bits
                if (byte_index == 3) begin
                    rom_mem[word_index] <= current_word | (rx_shift << 24);
                    current_word <= 0;
                    byte_index   <= 0;
    ...
    // SAÍDA DE LEITURA (BOOT_MANAGER -> CPU)
    // O sinal rd_data agora tem um driver explícito conectado ao array
    assign rd_data = (rd_addr < MEM_WORDS) ? rom_mem[rd_addr] : 32'h0;
