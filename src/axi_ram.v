...
module axi_ram #(
    ...
    // =====================================
    // LÓGICA DE ESCRITA UNIFICADA
    // =====================================
    // Em hardware, a RAM só pode ter um controle de escrita por porta.
    always @(posedge clk) begin : MEM_WRITE_LOGIC
        integer j; // Variável de loop local para evitar conflitos
        
        if (boot_we) begin
            // Prioridade para o Bootloader
            mem[boot_addr[ADDR_WIDTH-1:2]] <= boot_wdata;
        end 
        else if (s_axi_wready && s_axi_wvalid && s_axi_awready && s_axi_awvalid) begin
            // Escrita via AXI
            for (j=0; j<DATA_WIDTH/8; j=j+1) begin
                if (s_axi_wstrb[j]) begin
                    mem[s_axi_awaddr[ADDR_WIDTH-1:2]][8*j +: 8] <= s_axi_wdata[8*j +: 8];
                end
            end
        end
    end
