...
module soc_top (
  ...
  // --- Sinais internos para conexão dos periféricos ---
  //wire [31:0] irq; // nome de port exclusivo do picorv32
  wire [31:0] eoi;
  wire        trace_valid;
  ...
  //wire cpu_resetn;
  wire cpu_resetn_wire; // O reset que a CPU realmente vai usar  

  boot_manager #(.MEM_WORDS(256)) boot_mgr (
      .clk(clk),
      .resetn(resetn),
      .boot_mode(boot_mode),
...
picorv32_axi #(
      ...
      .ENABLE_IRQ    (1'b1) // Sugestão: Ative isso para o Timer funcionar
  ) cpu (
      .clk(clk),
      .resetn(cpu_resetn_wire),
      .trap(trap),
      ...
      // mem_axi_bresp REMOVIDO pois não existe no módulo
      ...
      // mem_axi_rresp REMOVIDO pois não existe no módulo
      
      .irq({31'b0, timer_irq}), // Conecte o timer aqui para evitar erro de "No Driver" no timer
      .eoi(eoi),
      .trace_valid(trace_valid),
      .trace_data(trace_data),

      // Conecte os outros pinos PCPI como você já fez...
  ...
  axi_ram ram_inst (
            ...
            .s_axi_bresp(ram_bresp),// opcional
            ...
            .s_axi_rresp(ram_rresp), // opcional
